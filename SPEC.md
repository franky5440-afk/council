# council 規格

版本 v0.1（2026-08-03）。本檔是實作的**唯一契約來源**；工作包只引用本檔，不重述。

---

## 1. 這是什麼

一個本機執行的 orchestrator，讓使用者在**同一個對話框**裡同時徵詢 3～4 個不同的 AI，
彼此看得見、輪流發言，最後由使用者指定的**仲裁者**整合出結論。

**核心設計決定：council 不串接任何模型 API。**
它驅動使用者自己安裝並登入的官方 CLI 子行程（`claude` / `codex` / `gemini` / `opencode`），
因此吃的是使用者自己的**訂閱額度**，而非計費 API。

這個選擇同時決定了三件事：

- council **不接觸、不儲存、不轉發任何憑證**。登入狀態由各家 CLI 自己管。
- 不繞過任何服務條款，因此本專案可以公開發布。
- 使用者必須自備並登入 CLI。目標客群是清楚這件事、也清楚**這會大量消耗自己額度**的進階使用者。

### 非目標（v1 明確不做）

- 不做帳號代管、不做登入引導自動化
- 不做 API key 模式（即使技術上更簡單）
- 不做手機端
- 不做討論內容的雲端同步

---

## 2. 名詞

| 名詞 | 定義 |
|---|---|
| 席次 seat | **一個 `(CLI, 模型)` 組合**。席次才是議會的成員單位，不是 CLI |
| 顧問 advisor | 參與討論的席次，**1～3 個** |
| 仲裁者 arbiter | 指定一個席次擔任。**不參與討論**，但看得見全部發言 |
| 席次總數 | **2～4**（顧問 ＋ 恰好一個仲裁者）。顧問上限 3 是這條除出來的，不是另一條規則 |
| 一輪 round | 每位顧問各發言一次，順序固定 |
| 逐字稿 transcript | 由 council 自己保存的完整發言紀錄，是組 prompt 的唯一依據 |

### 2.1 一個 CLI 可以提供多個席次

**議會的成員是席次，不是 CLI。** 四個 CLI 都支援 `--model` 指定模型，
因此同一個 CLI 可以同時佔用多個席次、各跑不同模型。

2026-08-03 實測：`opencode` 以 `-m` 分別跑 `deepseek-v4-flash-free` 與
`nemotron-3-ultra-free`，兩者各自回報不同身分，互不干擾。

兩個直接後果：

- 使用者可以組出**完全由免費模型構成的議會**（例如 opencode 之下的數個 free 模型），
  不動用任何付費訂閱。
- 開發與測試時，`ask()` 的驗證可以全部跑在免費席次上，**不消耗任何人的付費額度**。

### 2.2 席次必須顯式指定模型，且必須揭露實際回答者

⚠️ **不指定模型是危險的預設。** 2026-08-04 實測：未帶 `--model` 呼叫 `claude` 時，
它採用了使用者 `~/.claude/settings.json` 裡的偏好（實測為 `claude-opus-5[1m]`）。
也就是說**同一份 council 設定，在不同人的機器上會由不同的模型回答，而使用者不會知道**。
這與「議會由誰組成」這個核心概念直接抵觸——不知道誰在說話的議會沒有意義。

因此：

1. **模型為選填。** 留空代表沿用該 CLI 自己設定檔的偏好——這是常態路徑。
   本產品面向進階使用者，他們本來就會在各家 CLI 的 config 設好偏好模型，
   強制填寫是替使用者製造工作。README 需說明各家 config 的位置。
2. **每則發言都要記錄並顯示實際回答的模型**，取自各 CLI 自己的輸出，而不是回顯我們送出的值。
   **安全性由「顯示實際回答者」提供，不由「強制事先指定」提供。**
3. **指定值與實際值不一致時，UI 必須明確警示**——那代表 CLI 換掉了我們要求的模型。
4. ⚠️ **但「模型」這個欄位本身不可省略**——因為 CLI 的設定檔是**全域**的，
   一家 CLI 只會有一個偏好模型。§2.1 規定席次（不是 CLI）才是議會成員，
   **同一個 CLI 要開多個席次跑不同模型時，只能靠這個欄位區分**。
   拿掉它 ⇒ 一家 CLI 只能出一個席次 ⇒ §2.1 那個「由多個免費模型組成的議會」
   就組不出來了。**同一 CLI 開兩席以上時必須指定模型**，否則兩席會是同一個模型，
   議會等於少一個人。

#### 各 CLI 的模型可見性（2026-08-04 實測，能力不對稱）

| id | 可列出模型 | 實際模型回報位置 | 可靠度 |
|---|---|---|---|
| `claude` | ✗ 無列出指令 | stdout JSON 的 `modelUsage` 鍵與 `canonicalModel` | 可靠 |
| `codex` | ✗ 無列出指令 | **僅 stderr** 的 `model: <名稱>` 一行 | 脆弱，格式無保證 |
| `gemini` | ✗ 無列出指令 | stdout JSON 的 `stats.models.<名稱>` 鍵 | 可靠 |
| `opencode` | ✓ `opencode models` | **完全不回報** | 取不到 |

兩個後果必須誠實面對，不得用推測填補：

- **模型選擇一律是純文字欄位，v1 不做選單、不做模型探索。** 雖然 `opencode models`
  能列出候選（免費、即時），其餘三家都沒有對應指令 ⇒ 做出來的選單會是四家不一致的
  半套功能，而目標使用者本來就知道自己要用哪個模型。輸入錯誤由 `ask()` 回傳可讀錯誤。
  **更不得在程式中硬寫模型清單**——那會過期，且與 §4.1 的同一條理由衝突。
- **`opencode` 席次的實際模型無法查證**，UI 只能顯示「指定值（未經回報確認）」。
  ⚠️ 不可因為取不到就顯示成已確認——那是把 Unknown 偽裝成 Evidence。

---

## 3. 架構

```
 使用者 ─▶ 本機服務（討論引擎）─▶ Adapter ─▶ CLI 子行程 ─▶ 各家訂閱
              │
              └── transcript（council 自己保存）
```

### 3.1 關鍵決定：Adapter 無狀態，逐字稿由 council 自己管

每次呼叫 CLI 都是**獨立、無狀態**的一次性請求；council 自己把逐字稿組進 prompt，
**不使用各家 CLI 的 session 續談功能**。理由：

1. `gemini` 沒有續談旗標，四家語意不統一，硬要統一會做出最脆弱的那一層。
2. 「誰看得到什麼」必須由 council 精確控制——仲裁者不參與討論卻要看得見全部發言，
   這個需求在「我們自己組 prompt」的前提下是免費的，靠 CLI session 反而做不到。
3. 無狀態的 adapter 可以獨立測試，壞掉時責任邊界清楚。

代價：每輪重送逐字稿，token 消耗較高。**接受**——使用者吃的是訂閱額度，
且第 5 節的邊界會把總量壓在可控範圍。

### 3.2 逐字稿一律經 stdin 送出，不走 argv

**逐字稿放進 argv 會撞上作業系統的硬上限。** Linux 的 `MAX_ARG_STRLEN` 限制單一
參數約 131,072 **位元組**（不是字元）。中文一字 3 bytes ⇒ §5 的 8000 字元發言
＝ 24,000 bytes ⇒ 累積 6 篇就爆，**第 2 輪就死**；英文也撐不到第 4 輪結束。
這使 §3.1 的「重送完整逐字稿」與 §5 的「5 輪上限」在 argv 傳輸下**互相矛盾**。

**正解：把逐字稿寫進子行程的 stdin，argv 只帶一句簡短指示。**
2026-08-04 實測四家 CLI 全部可行——各以 150,000 bytes 中文餵入：

| id | 送達 token | 結果 |
|---|---|---|
| `claude` | 61,883 | 正確回覆，8.4 秒 |
| `gemini` | 59,991 | 內容送達（exit 0），126.8 秒 |
| `opencode` | 50,073 | 正確回覆，7.8 秒 |
| `codex` | 短測通過 | stderr 明示 `Reading additional input from stdin...`；長測未做 |

⚠️ **這推翻了先前「stdin 一律導向 `/dev/null`」的規則。** 該規則的理由是「四家都會
讀 stdin，不關掉會讓行程卡住」——但實測證明**「餵完內容後關閉」與「開著不給東西」
是兩回事**：8 次真實呼叫全部正常結束，無一卡住。正確做法是寫入後關閉 stdin，
而不是不給。

⚠️ **殘留未知（不得當成已驗證）**：`codex` 的長內容（150 KB）未實測；`gemini`
雖確認送達 60K token，但當次回覆內容失控（探測用的填充是同一字元重複五萬次，
屬病態輸入），**「長逐字稿能被正常運用」對 gemini 仍未證實**。兩者要補驗才能升級。

### 3.3 顧問看不到專案，脈絡必須由 council 顯式注入

⚠️ **顧問對使用者的專案一無所知。** §4.2 的唯讀保證是靠「把顧問關進一個只有 agent
定義檔的臨時目錄」達成的（`opencode` 的 `--dir <tempdir>`、`codex` 的
`--sandbox read-only`）——它們**讀不到原始碼、讀不到本文件**。顧問看得到的東西
**只有 council 送進 prompt 的內容**：專案脈絡、原始問題、逐字稿。

2026-08-05 實測證實這會直接毀掉答案品質：以「這個專案接下來該先做仲裁還是先做
web UI」提問，三席顧問全部把「仲裁」理解成業務領域的爭議仲裁，整場討論談的是
OpenAPI 契約與利害關係人。**其中一席主動標明**「工作目錄裡只有顧問設定檔，沒有專案
原始碼或規格書……若專案實際情況不同，請以事實為準」——模型誠實地標了自己的未知，
是**出題的人沒給脈絡**。

⇒ **`Discussion` 接受一段選用的 `context` 字串**，非空時由 §6 的 prompt 組成放在
最前面，作為獨立區塊。規則：

- **脈絡在最前、問題其次、逐字稿再次、任務最後。** 脈絡是背景，先於問題出現才自然。
- **每一位顧問、每一輪都重送完整脈絡**（與逐字稿同一原則，§3.1）。顧問是無狀態的，
  沒有「上一輪已經告訴過它」這回事。
- ⚠️ **脈絡會顯著放大每次呼叫的 token**，且每輪、每席次重複計費。
  §5 邊界 6 的用量顯示因此更重要，不是可有可無的裝飾。
- ⚠️ **council 不替使用者挑要送什麼**：不做自動摘要、不自動掃描專案、不猜哪些檔案
  相關。要送什麼由使用者指定，送多少由使用者負責。自動挑選會做出一個「看起來知道
  你的專案、其實只讀了三個檔」的東西——那比誠實的空白更危險。
- 🔴 **脈絡會原文送出這台機器，送到你指定的每一家 AI 廠商，每輪每席次各送一次。**
  因此 `--context` 是本工具唯一能把本機檔案外送的管道：指到 `.env`、憑證檔、
  金鑰、含個資的 log，就等於把它們交出去，而且**送出後收不回來**。
  這條與上一條是同一個原則的兩面——**由使用者挑選，代價也由使用者承擔**，
  所以更要在此明文寫出代價是什麼。

  ⚠️ **不要用「自動偵測敏感檔名並攔截」來解決這件事。** 理由與上一條相同：
  比對 `.env`／`*.key`／`credentials` 這類清單必然漏（`config.local.json`、
  `backup.sql`、貼著 token 的 `notes.md` 都不會命中），而**一個會漏的攔截器最大的
  害處是讓使用者以為有人在把關**，於是不再自己看一眼。誠實的做法是：council
  完全不猜，把「這個檔能不能外送」留在使用者眼前，並在文件與 `--context` 的
  說明文字裡講清楚。

---

## 4. Adapter 介面契約

每個 CLI 一個 adapter，**只准實作這兩個函式**，不得增加公開介面。

```python
def detect() -> dict:
    """偵測此 CLI 是否可用。不得執行任何會消耗額度的呼叫。
    回傳：
      {"id": str, "installed": bool, "path": str|None,
       "version": str|None, "error": str|None}
    """

def ask(prompt: str, model: str, timeout_s: int, max_chars: int) -> dict:
    """送出一次性請求並取回純文字答覆。
    model=None 代表沿用該 CLI 設定檔的偏好，此時**不得帶上 --model 旗標**；
      為字串時以該 CLI 的 --model 旗標指定（理由與但書見 §2.2）。
    必須：以子行程呼叫 CLI 的非互動模式；**prompt 經 stdin 送出**（見 §3.2），
          argv 只帶固定的簡短指示；寫入後關閉 stdin；逾時強制終止；
          輸出超過 max_chars 時截斷並標記 truncated=True。
    不得：續用 session、寫入專案外檔案、要求互動確認、把 prompt 放進 argv。
    回傳：
      {"ok": bool, "text": str, "truncated": bool,
       "error": str|None, "elapsed_s": float,
       "model_used": str|None, "usage": dict|None}
    """
```

**`model_used` 與 `usage` 的取得規則**（§2.2 的可見性、§5 邊界 6 的額度可見性都依賴它）：

- `model_used`：從該 CLI 自己的輸出取得實際回答的模型，**不得回顯我們送出的 `model` 值**
  ——回顯無法偵測 CLI 換掉模型。取不到就回 `None`，由 UI 標示為未經確認。
  各家的取得位置見 §2.2 的表格；`opencode` 不回報，一律 `None`。
- `usage`：token 用量與成本，取自各 CLI 輸出中既有的統計欄位（四家皆提供）。
  取不到回 `None`。**不要自行估算 token**——估錯比沒有更糟。

⚠️ 傳輸方式（stdin vs argv）是 adapter 的**內部細節，不得外露到呼叫端**。
引擎只負責組出 prompt 字串，不需要知道它怎麼送進子行程；四家的差異必須止於 adapter。

⚠️ 現有程式碼與本契約的落差，實作工作包必須一併處理：`ask()` 目前把 prompt 放進
argv；`base.run()` 寫死 `stdin=DEVNULL`；`base.MAX_ARG_CHARS` 以 `len(prompt)`
比對**字元**而非位元組（中文 3 bytes/字 ⇒ 對中文完全失效，10 萬中文字元＝30 萬
bytes 會通過檢查然後 exec 失敗，實測 `Errno 7`）。

⚠️ **prompt 改走 stdin 之後，長度檢查絕不可再套用在 `prompt` 上。** stdin 沒有
`MAX_ARG_STRLEN` 限制（實測 180,000 bytes 中文完整送達），對 prompt 設 argv 級別
的長度上限會**擋掉 stdin 完全承受得住的長逐字稿，等於讓本次改造失效**。
argv 上只剩固定的簡短指示與旗標，長度風險已經消失；使用者可控而會進 argv 的只有
`model` 字串（其注入風險由既有的 `check_arg_injection` 負責）。
發言長度的節制屬 §5 邊界 2 的 `max_chars`，與 argv 上限是兩件事，不要混為一談。

### 4.1 各 CLI 的非互動呼叫方式（2026-08-03 實測）

| id | 指令 | 指定模型 | 唯讀／限制 | 回覆文字在哪 |
|---|---|---|---|---|
| `claude` | `claude -p <prompt> --output-format json --tools ""` | `--model` | `--tools ""` 停用全部內建工具 | JSON 的 `result` |
| `codex` | `codex exec --sandbox read-only --skip-git-repo-check --output-last-message <檔案> <prompt>` | `-m` | `--sandbox read-only`，`-C` 圈住目錄 | 該檔案的純文字 |
| `gemini` | `gemini -p <prompt> -o json --approval-mode plan --skip-trust` | `-m` | `--approval-mode plan` | JSON 的 `response` |
| `opencode` | `opencode run --format json --agent advisor <prompt>` | `-m`（`provider/model`） | 自訂 agent：`"*": deny` 後單開 `websearch`（見 §4.2） | 事件流中 `type=="text"` |

2026-08-03 **實機呼叫**四家各一次驗證上表；真實輸出樣本存於 `tests/fixtures/`，
擷取過程中發現的坑記於該目錄的 `README.md`，實作前必讀。其中兩點會直接讓呼叫失敗：

- **stdin 必須寫入 prompt 後關閉**（見 §3.2）。⚠️ 本條 2026-08-03 原寫「一律導向
  `/dev/null`」，理由是四家皆會讀 stdin、不處理會讓 `claude` 卡住（實測單次呼叫由
  9.9 秒暴增為 176 秒才被終止）。**那個結論下得太寬**：卡住的成因是「開著卻不給
  內容」，而非「有 stdin」。2026-08-04 改以管道餵入內容後關閉，四家 8 次真實呼叫
  全部正常結束。⇒ **不給內容時導向 `/dev/null`，要送 prompt 時寫入後關閉**，
  兩者都不可讓 stdin 開著空等。
- **`gemini` 必須同時給 `--skip-trust`**，否則在不受信任的目錄會以 exit 55 中止，
  且 `--approval-mode plan` 會被**無聲**改回 `default`——唯讀保證會在使用者不知情下失效。

⚠️ **這些旗標是 2026-08-03 當天的事實，不是永久契約。** CLI 改版會變。
adapter 必須在 `detect()` 取得版本，並在 `ask()` 失敗時回傳可讀的錯誤，
而不是讓使用者看到一坨 stderr。

### 4.2 顧問一律唯讀

顧問的職責是出意見，不是動手。所有 adapter 必須以各 CLI 提供的**機制層**手段
限制工具與檔案存取，不得只靠 prompt 拜託模型別亂動。

#### `opencode` 的唯讀怎麼做（2026-08-03 實測確定）

⚠️ **`--dir` 不是可靠的權限邊界。** 曾誤記為「`--dir` 圈住工作目錄」，那是錯的——
`opencode run --help` 對它的說明只有 "directory to run in"。

實測（同一探測重現 3 次全部成功）：`--dir` 指向空白暫存目錄後，opencode 仍能用
bash 寫檔到該目錄**之外**，檔案確實建立。

⚠️ 更精確地說，邊界**存在但會漏**，這比「完全沒有邊界」更難察覺：
同一次呼叫的 stderr 會出現 `permission requested: external_directory (...); auto-rejecting`
——權限系統確實有動作——但寫入**仍然成功**。對照組：要求它以 `bash cat` 讀取
同一個外部檔案時**確實被擋下**。因此 `external_directory` 這道閘門對讀取有效、
對 bash 寫入無效，不能用它來論證「子行程被關在 `--dir` 內」。

⚠️ **內建的 `--agent plan` 也不夠。** 其權限清單中 `edit` 為 `deny`，但**沒有任何
`bash` 條目**，bash 因而落在 `*: allow` 之下。實測它沒寫成檔案，但模型的回覆是
「目前處於 Plan Mode，不能執行寫檔操作」——那是**模型自願遵守**，屬 prompt 層，
正是本節禁止依賴的東西。換一個不那麼配合的模型就不成立。

**正解**：由 council 在當次的暫存目錄內寫一個自訂 agent 定義
`<暫存目錄>/.opencode/agents/advisor.md`，以 frontmatter 的 `permission` 區塊
**先用萬用字元全部關掉，再單獨開需要的那一個**，再以 `--agent advisor` 執行：

```yaml
permission:
  "*": deny
  websearch: allow
```

🔴 **必須是「全關再單開」，不是「逐項 deny」。** 2026-08-06 之前這裡是一份列舉清單
（`bash`／`edit`／`webfetch`／`task`／`todowrite`／`websearch`／`lsp`／`skill`），
於是**沒被列到的工具落在 `*: allow` 之下** —— 實測發現顧問因此一直握有
`read`／`grep`／`glob`，餵它一個專案內檔案的絕對路徑，它**真的會去呼叫 `read`**。
當時擋下它的是 opencode 的 `external_directory` 閘門，而本節上面就記載了那道閘門
**對 bash 寫入無效** ⇒ 唯讀保證有一段是靠一道已知會漏的閘門在撐。
列舉清單還有一個更難察覺的問題：**opencode 將來新增任何工具，都會自動變成 allow**。
萬用字元版是 fail-closed 的，新工具預設關閉。

實測（2026-08-06，三個免費模型）：`"*": deny` 生效且**單項 `allow` 勝過萬用字元**——
websearch 正常呼叫，而檔案讀取工具**根本沒有被掛載**（模型自述「只掛載了 websearch
一個工具」），canary 檔案未外洩，也沒有退回預設 agent。

更早的實測（2026-08-03）確認此法確為機制層生效：事件流中**完全沒有 bash 工具呼叫**
（該工具不存在），stderr 出現
`permission requested: external_directory (...); auto-rejecting`，目標檔案未被建立。

#### 為什麼 `websearch` 是 `allow`

顧問的職責是出意見，而**「查得到現況」與「唯讀」是兩件事**。2026-08-06 實測：
同一個「比較兩家 CLI 訂閱方案」的問題，關掉搜尋時三席顧問全部只能憑訓練資料推論，
第一位對產品是什麼**猜錯了**，後兩位在「互相回應」的要求下沿用了那個前提 ⇒
看起來像三人達成共識，實際上是同一個未經查證的假設被複述三次。開啟搜尋後，
三席都會實際發出查詢並帶回可查證的來源。

⚠️ **代價要說清楚**：顧問由此獲得對外的網路出口。使用者貼進來的脈絡片段有可能被
模型寫進搜尋查詢、送給搜尋供應商。這是**同一道信任邊界的延伸而不是新開一道**
——脈絡本來就已經整份送給雲端模型了——但它確實多了一個接收方。
🔴 **`webfetch` 仍然是 `deny`**：搜尋是「送出查詢字串」，webfetch 是「對任意 URL
發請求」，後者等於給模型一條主動把脈絡送到它指定位置的路。兩者不可混為一談。

⚠️ **搜尋會把 token 用量放大約一個數量級**（2026-08-06 實測，同一個免費模型）：
不搜尋的一則發言是 `tokens.total: 6,542`，搜尋過的是 **57,511**（其中
`cache.read: 38,528`）——搜尋結果會整份進 context。免費席次成本仍是 0，但
**付費席次要把這個放大效應算進 §5 邊界 6 的額度估計**。

⚠️ **這個放寬只發生在 `opencode` 這一家。** `claude` 是 `--tools ""`（一個工具都沒有）、
`codex` 是 `--sandbox read-only`、`gemini` 是 `--approval-mode plan`，它們都沒有搜尋。
⇒ **同一場議會裡，不同 CLI 的席次能不能查證現況是不一樣的**，問到需要即時資訊的
題目時，各席答案不可直接比較——這與 §2.2「opencode 會注入自己的環境資訊」是同一類
的不可比性，出題與讀結果時都要記得。

⚠️ **但 `--agent` 是 fail-open 的，必須自行補上失敗關閉。** 實測 `--agent` 指向
不存在的 agent 時，opencode **無聲退回完全可寫的預設 agent**、exit code 仍為 0、
檔案照樣被寫出，唯一訊號是 stderr 的一行
`agent "..." not found. Falling back to default agent`。
因此 adapter **必須**檢查 stderr 是否出現該退回訊息，出現即回 `ok=False`——
不得讓唯讀保證在無人察覺的情況下消失。

⚠️ 本專案自己的 `dispatch.sh` 同樣只用 `--dir`，所以 builder 被「關在 council
目錄內」這個說法**沒有機制層保證**。派工時要當成 builder 有能力寫到任何地方。

---

## 5. 停止邊界（本專案最重要的一節）

前提：AI 看到 input 就會產出 output，不會自己收斂。因此邊界必須由 council 施加，
且**機制層優先於 prompt 層**。共六道：

1. **輪與輪之間一定要人。** 一輪之內顧問自動連續發言（這是「熱鬧」的來源），
   但一輪結束後**永不自動進入下一輪**，等使用者按「再一輪」或「叫仲裁者」。
   這是最關鍵的一道——它把失控的可能性直接交回人手上。
2. **單次發言長度上限**（`max_chars`，預設 8000 字元）。超過即截斷並標記，
   避免一家把後面的人 context 撐爆。
3. **輪數硬上限**（預設 5 輪）。達上限後只剩「叫仲裁者」或「明確確認再開一輪」兩條路。
4. **逾時強制終止**（`timeout_s`，預設 **180 秒**）。子行程逾時即 kill，該顧問本輪記為
   「未回應」，討論繼續，不整場卡死。

   預設值原為 120 秒，2026-08-03 實測後調整為 180：`gemini` 遇到 503 會**自行重試並退避**，
   實測兩次**成功**的呼叫分別耗時 **119.5 秒**與 **102.5 秒**，前者差 0.5 秒就會被舊上限
   砍掉、並被誤記為「未回應」。證據見 `tests/fixtures/gemini_success.json` 的
   `stats.models.*.api`（`totalRequests: 3`、`totalErrors: 2`）。
   代價是卡住的顧問要多等一分鐘；但「把成功的回覆誤殺並謊報未回應」比多等更糟。
5. **收斂偵測**：每位顧問回覆結尾必須輸出一行
   `[立場: 同意|保留|反對] [補充: 有|無]`。
   全體皆「補充: 無」時，UI 提示可以收斂了。
   模型沒輸出這行 → 保守視為「補充: 有」並記錄一次格式違規（不重試、不懲罰）。

   **解析規則**（與 §2.2 取模型名同一條原則：**錨定位置，不要掃全文**）：

   - **只認整段回覆的最後一個非空白行。** §6 規定 prompt 由其他顧問的逐字發言組成
     ⇒ 逐字稿裡本來就會出現別人的這一行，模型也可能順手複述。掃全文等於讓別人的
     立場冒充這一位的立場。出現在別處的標記一律忽略，視同沒輸出。
   - 冒號接受半形 `:` 與全形 `：`，**方括號只接受半形**。中文輸出打出全形冒號是常態，
     為此判成格式違規會讓收斂訊號永遠不亮；但再往下放寬就變成猜，停在這裡。
   - 發言被 `max_chars` 截斷（邊界 2）時這行會跟著被切掉 ⇒ 自然落入「格式違規、
     補充: 有」。這是正確的保守結果，不要為它加補救。
   - 顧問逾時未回應（邊界 4）一樣視為「補充: 有」——沒發言的人不能算收斂；
     但**不計入格式違規**，那是呼叫失敗，不是模型不守格式，混在一起會汙染統計。
6. **額度可見性**：UI 常駐顯示本次討論的用量（總計與各席次分計），**不只是呼叫次數**。
   既然會大量燒額度，使用者必須隨時看得到自己燒了多少。

   四家 CLI 的輸出**本來就附帶 token 與成本統計**（見 §4 的 `usage`），不顯示等於
   浪費現成資料。至少要有：呼叫次數、輸入／輸出 token、CLI 自報的成本（若有）。

   ⚠️ 實測發現這些數字比直覺大得多：`claude` 一次僅 30 字元的呼叫就回報
   `cache_creation_input_tokens: 15,334`、`total_cost_usd: 0.153725`——**每次呼叫
   都有約 1.5 萬 token 的固定系統提示開銷，與問題長短無關**。
   5 輪 × 4 席次＝20 次呼叫，光固定開銷就約 30 萬 token，逐字稿還沒算。
   這正是本條存在的理由。

   ⚠️ `opencode` 的免費模型回報 `cost: 0`，用量顯示對它意義不大；
   成本呈現的重點放在 `claude` / `codex` / `gemini` 三家。

---

## 6. 討論流程

1. 使用者設定 2～4 個模型，指定其中一個為仲裁者（仲裁者**不列入**顧問輪替）。
2. 使用者提問。
3. 一輪開始，顧問依固定順序逐一發言。第 N 位收到的 prompt 包含：
   原始問題 + 本輪前 N-1 位的發言 + 先前各輪完整逐字稿。
4. 一輪結束，停下來等使用者（邊界 1）。
5. 使用者可：再開一輪 / 追問 / 叫仲裁者。
6. 仲裁者被呼叫時，收到完整逐字稿與原始問題，輸出整合結論。
   仲裁者**不曾**參與任何一輪發言。

### 6.1 仲裁的前提與記帳

仲裁**不是第四位顧問**。它不進輪替、不進逐字稿、不參與收斂偵測，
而是使用者在一輪結束後可以選擇觸發的**一次獨立呼叫**。

- **前提有三條，全部成立才能仲裁**：① 不在進行中的一輪 ② 至少完成一輪
  ③ 逐字稿裡至少有一則成功的發言。
  前兩條是為了逐字稿完整——半輪的逐字稿會讓仲裁者對著殘缺的討論下結論，
  而它無從得知自己看到的是半場。第三條是為了額度：全體顧問都未回應時，
  仲裁者收到的是一份空討論，那次呼叫必然浪費（`claude` 一次呼叫的固定開銷
  約 1.5 萬 token，見 §5 邊界 6）。
  ⚠️ 這三條是**機制層**的檢查，不是 UI 上的提示。它必須擋在真正發出呼叫之前——
  先花錢再發現狀態不對，錢已經燒掉了。
- **仲裁者的用量必須併進本次討論的累計**（呼叫次數與 token 皆是）。
  漏掉它，§5 邊界 6 的顯示就會少報最貴的那一次：`claude` 當仲裁者時，
  一次仲裁的固定開銷與一位顧問發言同級，而它又拿著最長的逐字稿。
- **仲裁者不套用立場標記解析**（§5 邊界 5），也不計入格式違規統計。
  收斂講的是「顧問還有沒有話要說」，仲裁者不在那個集合裡；把它算進去會讓
  收斂訊號與格式違規數同時失真。因此**也不該要求仲裁者輸出那一行**。
- **仲裁不改變輪次狀態**。仲裁完成後使用者依然可以再開一輪（仍受邊界 1、3 約束）。
  它不是討論的終止符，是隨時可取用的一次整合。

---

## 7. 技術選型

- **Python 3.10 以上**，標準函式庫優先，非必要不加依賴。理由：本機是 2012 iMac，
  避免建置步驟與重框架；同層 `lottrey` 亦為 Python。
  ⚠️ **下限是 3.10 而不是「Python 3」**：`src/adapters/` 用 PEP 604 的 `X | None`
  型別註解（`base.py:11,106,107` 與四個 adapter 的 `ask()`，共 7 處），且**刻意不加**
  `from __future__ import annotations`。舊版直譯器會在 import 階段就丟
  `TypeError: unsupported operand type(s) for |`。
  ⚠️ macOS 內建的 `/usr/bin/python3`（Xcode Command Line Tools）常是 3.9.x，**不足**。
- 本機服務：`http.server.ThreadingHTTPServer` + SSE 推送。討論本身是序列的，
  併發需求極低，不值得為它引入 web 框架。
- UI：單頁 HTML + 原生 JS，無建置步驟。
- 桌面殼（Tauri / Electron）**延後決定**，先確保核心在瀏覽器可用。

### 7.1 討論活在伺服器記憶體裡（這不是存檔）

命令列一次執行＝一輪，狀態隨行程結束消失，所以第二輪做不到。web UI 要多輪，
就必須有一層讓同一個討論**跨請求存活**。

- **只在記憶體。伺服器一停，討論就沒了。** 這**不是** §8 延後的「討論存檔與匯出」——
  沒有檔案格式、沒有回復、不承諾跨重啟。刻意如此：一旦落檔就要面對格式版本、
  清理策略，以及「逐字稿裡含使用者自己送進去的專案脈絡」（§3.3）該不該留在磁碟上，
  那是另一個要單獨做的決定，不該搭著 UI 順便發生。
- 🔴 **同一個討論同時只能有一個請求在動它。** `ThreadingHTTPServer` 是多執行緒的，
  上面「討論是序列的、併發需求極低」講的是**不需要 web 框架**，不是不會有兩個執行緒
  同時進來：兩個分頁同時按「再一輪」，兩條執行緒會**同時通過 §5 邊界 1 的相位檢查**，
  於是跑出兩輪、花兩倍的錢。邊界 1 是本專案最關鍵的一道，它不能被多執行緒繞過。
- **執行權必須是非阻塞地取得。** 搶不到就立刻回覆「正在跑」，
  **不要排隊**——一輪可能跑 180 秒 ×N 席，讓第二個請求等完才發現輪次早被別人開走，
  是把使用者晾在那裡，還可能在他放棄之後才真的跑起來。
- **讀狀態不需要執行權，但讀到的是最後一次一致的快照。** 邊讀邊改會讀到半更新的
  用量統計（`status()` 會走訪各席次的累計字典）。輪次進行中的即時進度走事件流，
  不靠讀進行中的狀態機。
- **事件流可重放**：每個事件帶單調遞增的序號，客戶端拿著游標來要「這之後的」。
  筆電睡著、SSE 斷線是常態；重連時要能補上斷線期間的發言，而不是整場重來。
- 🔴 **session id 必須不可猜**（用 `secrets`，不是流水號）。它會出現在 URL 裡，
  而本機上**任何**程序都打得到這個伺服器；id 猜得到，就有人能替你開新一輪、
  花你的訂閱額度。這是本專案第一次有「別人能觸發花錢」的介面。

### 7.2 這個伺服器是一個會花錢的介面

上一條的理由——「本機上任何程序都打得到這個伺服器」——還有幾個後果沒被涵蓋。
它們一併寫在這裡，而且**全部是機制層，不是 UI 上的提示、也不是使用說明**：

- 🔴 **只綁 `127.0.0.1`，而且這是程式碼層的白名單、不是預設值。**
  傳入其他位址（含 `0.0.0.0`）必須**拒絕啟動**。寫成「預設 127.0.0.1、但可設定」
  等於把「讓整個區域網路花你的訂閱額度」放在一個旗標後面，那不是邊界，是預設值。

- 🔴 **花不花錢由行程啟動時決定，不由請求決定。** 真實呼叫（§4 的 adapter）在啟動時
  注入，與 `cli.py` 的 `--live` 同一個原則：**沒有任何請求能把 dry run 切成 live。**

- 🔴 **要防的不是「本機的程序」，是「使用者另一個分頁打開的網站」。**
  本機程序能直接打這個埠，這件事防不了也不該防——那是使用者自己的機器，
  等同他自己的終端機。真正的新風險是惡意網頁：它一行
  `fetch('http://127.0.0.1:<port>/…')` 就能替使用者開一輪、花他的額度。
  五道機制層檢查，任何一道不過就拒絕：

  1. **`Host` 標頭必須是 `127.0.0.1:<port>` 或 `localhost:<port>`**，否則拒絕。
     這道擋的是 DNS rebinding：攻擊者的網域解析到 127.0.0.1 時，
     瀏覽器送出的 `Host` 仍是他的網域。
  2. **`Origin` 標頭若存在，必須是我們自己的來源**，否則拒絕。
  3. **所有 `POST` 請求的 `Content-Type` 必須是 `application/json`**，否則拒絕。
     這會強迫跨來源請求先做 CORS preflight，而——
     ⚠️ **條件是「所有 POST」，不是「有 body 的請求」。** 本條原本寫成後者，
     而 `/rounds` 與 `/arbitration` 本來就接受空 body ⇒ 惡意網頁的
     `fetch(url, {method:'POST'})` 是一個不需要 preflight 的 simple request，
     這道防線對兩個最花錢的端點等於不存在（實測當時真的跑了一輪）。
     **安全檢查的條件要直接寫出要保護的對象，不要寫成一個剛好在多數情況下等價的代理條件。**
  4. **回應永遠不帶任何 `Access-Control-*` 標頭。** preflight 過不了，
     瀏覽器就替我們把它擋在發出去之前。
  5. **回傳 HTML 的回應必須拒絕被內嵌**（`X-Frame-Options: DENY` ＋
     `Content-Security-Policy: frame-ancestors 'none'`，兩者都要，且**必須是回應標頭**）。
     前四道防的都是「跨來源 `fetch()`」，但內嵌 iframe **繞過的是整組、不是其中任一道**：
     被框住的頁面自己發出的請求，`Host` 正確、`Origin` 同源、`Content-Type` 由頁面
     自己設、同源不需要 CORS 標頭——四道全部通過，而攻擊者只需要誘導點擊，
     從頭到尾不必讀到任何回應、也不必猜到 session id。
     ⚠️ **`frame-ancestors` 寫在頁面的 `<meta>` 裡無效**，瀏覽器只認回應標頭
     （`report-uri`、`sandbox` 同理）。頁面內的 `<meta>` CSP 管的是別的事，不能代替本條。

  ⚠️ 這五道是**互相支撐的一組**，不是五個獨立的選項；拿掉任何一道，
  另外四道各自都有繞過的方法。

- 🔴 **脈絡只接受請求裡的字串本文，伺服器不從路徑讀檔。**
  §3.3 說 `--context` 是本工具唯一能把本機檔案外送的管道。把「讀哪個路徑」
  變成一個 HTTP 參數，等於讓任何打得到這個埠的東西指定要外送哪個檔案——
  §3.3 講好的「由使用者挑選、代價由使用者承擔」會在使用者不知情下被別人代勞。
  要送什麼由使用者在自己的畫面上挑，**內容**經由請求帶進來，**伺服器不開檔**。
  - 同一個理由：**狀態查詢不回傳脈絡原文**，只回字元數。

- **讀狀態一律走 §7.1 的快照**，不對進行中的討論呼叫 `status()`。

---

## 8. v1 範圍

**做**：adapter 偵測與呼叫、序列討論引擎、六道停止邊界、仲裁流程、本機 web UI、
Linux/macOS 支援。

**延後**：Windows 支援、桌面殼打包、討論存檔與匯出、模型參數調整 UI。

⚠️ **加入 Windows 支援時必須一併處理**：`shutil.which()` 在 Windows 會把**當前工作目錄**
納入搜尋範圍（POSIX 不會）。屆時若使用者從一個含有惡意 `claude.exe` 的目錄啟動 council，
該執行檔會先於真正的 CLI 被解析到。v1 不支援 Windows，故現在不是漏洞；
但這一條不解掉就不能宣稱支援 Windows。（2026-08-03 security review 提出）

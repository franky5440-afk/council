...
- **模型選擇**：只有 `opencode` 能列出候選模型（免費、即時），可提供選單；
  其餘三家使用者得自行輸入模型字串，輸入錯誤由 `ask()` 回傳可讀錯誤。
  **不得在程式中硬寫模型清單**——那會過期，且與 §4.1 的同一條理由衝突。
...
def ask(prompt: str, model: str, timeout_s: int, max_chars: int) -> dict:
    """送出一次性請求並取回純文字答覆。
    model 為必要參數，不接受 None（理由見 §2.2：不指定會由各 CLI 的設定檔決定，
      使用者無從得知誰在回答）。以該 CLI 的 --model 旗標指定。
...
bytes 會通過檢查然後 exec 失敗，實測 `Errno 7`）。prompt 改走 stdin 後這道檢查
的守備範圍縮小到 argv 上的簡短指示，但**仍須改成以位元組計**。
